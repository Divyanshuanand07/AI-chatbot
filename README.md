...
Each one is a deliberate choice, not an accident:

| Deviation | Why |
|---|---|
| **Seven operational tables, not five.** Added `deliveries` and `order_documents`. | They carry most of the blocker signal. You cannot explain a stuck order without knowing which document was rejected or how many delivery attempts failed. |
| **Twelve tools, not six.** Added a composite summary, the rule engine, entity resolution, documents, finance and knowledge search. | `get_order_summary` collapses six round trips into one; `find_orders` lets an operator ask by phone or number plate instead of needing an order id; `search_knowledge_base` is what makes policy questions answerable at all. |
| **Root cause is a rule engine, not the model.** | A model handed raw order rows returns a fluent, plausible story that is sometimes wrong — and an operator will act on it. `diagnostics.py` is reproducible and reviewable. |
| **A stub LLM provider and a hashing embedder ship as defaults.** | The demo and the test suite run offline, deterministically, and at zero cost. Set `LLM_PROVIDER=anthropic` for the real thing. |
| **`orders.id` is the public identifier.** | Operators say "order #1243". Hiding the primary key behind a surrogate reference would mean translating every conversation. |
| **Ungrounded answers are flagged, not rewritten.** | Silently editing model output hides the failure. The warning surfaces in the response, the logs and the audit row, where someone can act on it. |

The product brief is not checked into this repo; the mapping above is drawn
from the constraints referenced in the code and seed scenarios.

---

## Limitations and next steps

Known gaps, in the order they would matter in production:

1. **Tool timeouts do not cancel the query.** `TOOL_TIMEOUT_SECONDS` stops the
   user waiting, but the query keeps running until Postgres gives up — Python
   cannot safely kill a thread. Real protection belongs in `statement_timeout`
   on the database role.
2. **Migrations run in the container entrypoint.** Fine for a demo, wrong for
   multi-replica deploys where concurrent `migrate` calls can deadlock. They
   belong in a release step that runs once.
3. **The knowledge corpus is seven documents, ingested by a management
   command.** No incremental re-index, no document upload API; editing a
   document means re-running `make knowledge`.
4. **No streaming.** A multi-tool turn against a live model legitimately takes
   tens of seconds and the caller waits for the whole thing. Server-sent
   events over the orchestrator loop is the obvious next step.
5. **Conversation context holds one resolved order id.** Enough for *"and what
   about its payment?"*, not enough for *"compare those two orders"*.
6. **Read-only by design.** The assistant cannot escalate, assign or update
   anything. Write actions would need an approval step and a different audit
   posture than the one here.
7. **The hashing embedder is a stand-in.** It is deterministic and free, which
   is what makes the suite hermetic, but it matches vocabulary rather than
   meaning. Production should run `EMBEDDING_PROVIDER=voyage` (its relevance
   floor, `0.50`, is already calibrated).